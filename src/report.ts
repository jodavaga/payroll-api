...
export function buildGlobalSummary() {
  const cycles = db.select().from(payrollCycles).all(); // no filtering by Status??
  ...
  for (const cycle of cycles) {
    const items = db.select().from(payItems).where(eq(payItems.payrollCycleId, cycle.id)).all(); // N+1 issue
    for (const item of items) {
      // Look up the employee to attribute this line item.
      db.select().from(employees).where(eq(employees.id, item.employeeId)).get(); // Where was stored the result?
